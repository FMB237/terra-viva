...
async def init_db():
    ...
    for table_sql in [candidates_schema, voters_schema, votes_schema, payments_schema, admins_schema, settings_schema]:
    ...
    settings_data = [
        ...
        ('event_time', '13:00'),
        ('event_venue', 'Alliance Française de Garoua, Antenne de Maroua'),
    ...
    for key, value in settings_data:
        if is_sqlite:
            ...
            await db.execute("INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO NOTHING", [key, value])
    
    candidates_data = [
        (1, 'MAFOCK KINGNE PELAGIE', 'miss', 'Informatique et Télécommunication', 'Niveau 1', None,
        ...
        (2, 'ZAITOUNA ADAMA', 'miss', 'Sciences environnementales', 'Niveau 2', None,
        ...
        (3, 'NUCK CÉCILE', 'miss', 'Sciences environnementales', 'IC1', None,
        ...
        (4, 'ANGUE OBAM MANUELLA ANNA', 'miss', 'Génie civil', 'Niveau 1', None,
        ...
        (5, 'SIGNING FRANK BRONDON', 'master', 'Agriculture, Élevage et Produits Dérivés (AGEPD)', 'Niveau 4', None,
         'Entrepreneuriat.',
        ...
        (6, 'ISSA NADJE SAMOUPA', 'master', 'Agriculture, Élevage et Produits Dérivés (AGEPD)', 'Niveau 1', None,
        ...
        (7, 'ANAËL NDENGUE', 'master', 'Énergie renouvelable (ENREN)', 'IC2', None,
        ...
        (8, 'AMOUGOU METOGO ELYSÉE', 'master', 'Sciences environnementales', 'Niveau 1', None,
    ...
    for row in candidates_data:
        if is_sqlite:
            await db.execute("INSERT OR IGNORE INTO candidates VALUES (?,?,?,?,?,?,?,?,?,?,?)", list(row))
        else:
            await db.execute("""
    ...
    admin_password = '$2b$12$TB6Oq.ucMurr3duerrjKGufn8VkclzG.HcwGsfldH3s2fwH1w0/FO'
    if is_sqlite:
        await db.execute("UPDATE admins SET username='Miguel', password_hash=? WHERE username='admin'", [admin_password])
        await db.execute("INSERT OR REPLACE INTO admins(id, username, password_hash, role) VALUES(1, 'Miguel', ?, 'super_admin')", [admin_password])
    else:
        await db.execute("""
